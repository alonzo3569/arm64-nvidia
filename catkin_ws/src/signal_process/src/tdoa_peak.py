	...
	def __init__(self):
        ...
        def Iterate(self, event):
            ...
            if self.gcc == True:
                ...
                tau = self.peak_threshold(ch1, ch2)
        ...
        def peak_threshold(self, filtered_ch1, filtered_ch2):
           
            # Find peaks in filtered data
            locs_1 = find_pks(filtered_ch1)
            locs_2 = find_pks(filtered_ch2)

            # get first over threshold voltage index
            over_thres_ch1_list, = np.where(filtered_ch1[locs_1] > threshold * ch1_avg)
            over_thres_ch2_list, = np.where(filtered_ch2[locs_2] > threshold * ch2_avg)
            index_ch1_start = locs_1[over_thres_ch1_list[0]]
            index_ch2_start = locs_2[over_thres_ch2_list[0]]

            # Calculate time difference tau using yhh threshold method
            tau = (index_ch1_start - index_ch2_start)/fs
            #print(f'Time difference : {tau}')
            return tau
