...
class TDOA:
	...
	def __init__(self):
            ...
            self.mic_distance = rospy.get_param('~mic_distance', 0.6)
        ...
        def Iterate(self, event):
            ...
            if self.ch1_data_array.shape[0] >= self.tdoa_array_size and self.ch1_data_array.shape[0] == self.ch2_data_array.shape[0]:
                ...
                if (index_ch1.size != 0 and # over thres volts exist
                    index_ch2.size != 0 and 
                    len(over_voltage_ch1_front) == 0 and # over thres volts are not in cutoff range 
                    len(over_voltage_ch2_front) == 0 and
                    len(over_voltage_ch1_back)  == 0 and 
                    ...
                    self.tdoa_msg.basic_tau = basic_tau
        ...
        def cc(self, ch1, ch2, fs):
            cc = np.correlate(ch1, ch2, 'full')
            index = np.argmax(np.abs(cc))
            tau = float(fs - index)/ fs
            return tau, cc
