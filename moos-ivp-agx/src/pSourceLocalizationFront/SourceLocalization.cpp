...
bool SourceLocalization::Iterate()
{
  ...
  // Return if the source isn't in front of the vehicle
  if (tdoa11 > 90.0 || tdoa11 < -90.0 || tdoa21 > 90.0 || tdoa21 < -90.0)
  {
    m_seglist.set_label("1");       // label has to be the same to remove previous seglist
    m_seglist.set_active(false);    // VIEW_SEGLIST cannot only contain label
  ...
  double theta[2] = {theta11, theta21};
...
void SourceLocalization::cramer_formula(double m1, double m2, double k1, double k2, double &x, double &y)
{
  ...
  // For waypoint bhv update
  //  RETURN_UPDATES = "points=50,50 # speed = 1.5"
  string msg = "points=" + doubleToString(x) + "," + doubleToString(y);
  Notify("TRANSIT_UPDATES", msg);
...
void SourceLocalization::calculate_location(double theta[2])
{
  double m1, m2, k1, k2, result_x, result_y;
  get_m_k(m_osx_heron, m_osy_heron, theta[0], m1, k1); //here
  //cout << "m1 :" << m1 << endl;
  //cout << "k1 :" << k1 << endl;
  get_m_k(m_osx_duckieboat, m_osy_duckieboat, theta[1], m2, k2);
  //cout << "m2 :" << m2 << endl;
  //cout << "k2 :" << k2 << endl;
...
void SourceLocalization::check_location_number()
{
  ...
  if(m_calculate > 0) // m_calculate range : 1,2,3,4
  {
